...
locals {
  ...
  security_groups_ids_master = concat(
      var.security_groups_ids_master, 
  ...
  security_groups_ids_nodes = concat(
      var.security_groups_ids_nodes, 
      var.enable_default_rules == true ? [
...
resource "yandex_kubernetes_cluster" "kube_cluster" {
  ...
  master {
    ...
    security_group_ids = local.security_groups_ids_master ### TODO: too much rules for master
